...
const ORIG = "/home/claude/work/v2build/src";
const SITE = "/home/claude/work/v2build/out";
...
for (const p of PAGES) {
  ...
  if (p === "case-sberpravo.html") {
    /* здесь меню обязано измениться: оно теперь берёт названия из блоков,
       а пользователь переименовал три из них */
    const la = [...a.window.document.querySelectorAll("#projects .subnav a")].map(x => x.textContent.trim());
    const lb = [...b.window.document.querySelectorAll("#projects .subnav a")].map(x => x.textContent.trim());
    const diff = la.map((x, i) => [x, lb[i]]).filter(([x, y]) => x !== y);
    ok(p + " — в меню столько же пунктов", la.length === lb.length, la.length + "/" + lb.length);
    ok(p + " — изменились только переименованные блоки",
      diff.length === 3 && diff.every(([, y]) => y === "Дискавери"), JSON.stringify(diff));
  } else {
...
ok("расхождение языков не выросло", tot()[0] - tot()[1] === 1, tot().join(" / "));
...
console.log("\n13. Перевод");
{
  ...
  ok("расхождение языков показано в панели", !doc.querySelector(".crm-warn").hidden,
    doc.querySelector(".crm-warn").outerHTML);

  fire(w, doc.querySelector(".crm-bar [data-a='translate']"), "click");
  ok("сначала предлагается выровнять структуру", !!doc.querySelector(".crm-modal [data-a='align']"));
  const enWas = T3.en.blocks[13].subs.length, ruWas = T3.ru.blocks[13].subs.length;
  fire(w, doc.querySelector(".crm-modal [data-a='align']"), "click");
  ok("недостающее скопировано из русского", T3.en.blocks[13].subs.length === ruWas,
    enWas + " → " + T3.en.blocks[13].subs.length);
  ok("предупреждение погасло", doc.querySelector(".crm-warn").hidden);
  ok("следом открылся перевод", /Перевод/.test(doc.querySelector(".crm-sheet-h b").textContent));

  const rows = doc.querySelectorAll(".crm-tr input");
  ok("непереведённое собрано в список", rows.length > 0, rows.length + "");
  const first = rows[0];
  first.value = "Discovery";
  fire(w, doc.querySelector(".crm-modal [data-a='apply']"), "click");
  const enJson = JSON.stringify(T3.en);
  ok("перевод записан в английскую версию", enJson.indexOf("Discovery") >= 0);
  ok("русский не тронут", JSON.stringify(T3.ru).indexOf("Discovery") < 0);
  d.window.close();
}
