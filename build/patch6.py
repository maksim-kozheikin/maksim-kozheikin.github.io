...
document.addEventListener("click", e=>{
  ...
  if(dragged) return;                              // это было перетаскивание, а не щелчок

  /* Попали ли по картинке, решаем по координатам, а не по e.target:
     во время перетаскивания браузер переадресует щелчок на область
     показа, и тогда по цели не отличить картинку от фона. */
  if(onImage(e.clientX, e.clientY)){
    if(z===ZMIN) setZoom(z*ZSTEP, e.clientX, e.clientY, true);   // немного приблизить
    return;
  }
  // щелчок мимо картинки закрывает — но только пока не приближено
  if(z===ZMIN) closeLb();
});
...
lbStage.addEventListener("pointerdown", e=>{
  ...
  if(pts.size===1){
    ...
    if(z>ZMIN){                                    // захват нужен только для перетаскивания
